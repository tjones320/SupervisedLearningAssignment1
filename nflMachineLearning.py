...
#Import the dataset 
dataset = pd.read_csv('nfl.csv')
"""
Split the data into a training and a testing set
"""
print("begin partition")
train_features = dataset.iloc[0:, :10]
train_targets = dataset.iloc[0:, 10:]
"""
Check the accuracy
NA = 0
kickoff = 1
...
estimator = DecisionTreeClassifier(min_samples_leaf = 33)
plot_learning_curve(estimator, title,train_features, train_targets.values.ravel(), (0.25, 1.01), cv=cv, n_jobs=4)

title = "Learning Curves (K Neighbors Classifier K=3)"
cv = ShuffleSplit(n_splits=100, test_size=0.2, random_state=0)
estimator = KNeighborsClassifier(n_neighbors = 3)
plot_learning_curve(estimator, title, train_features, train_targets.values.ravel(), (0.25, 1.01), cv=cv, n_jobs=4)

title = "Learning Curves (K Neighbors Classifier K=10)"
cv = ShuffleSplit(n_splits=100, test_size=0.2, random_state=0)
...
title = "Learning Curves (Support Vector Clasifier Linear)"
cv = ShuffleSplit(n_splits=10, test_size=0.2, random_state=0)
